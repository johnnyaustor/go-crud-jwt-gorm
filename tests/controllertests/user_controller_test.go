...
func TestCreateUser(t *testing.T) {
	...
	for _, v := range samples {
		...
		if err != nil {
			fmt.Printf("cannot conver to json: %v", err)
		}
...
func TestUpdateUser(t *testing.T) {
	...
	samples := []struct {
		...
		},
		{
			...
			id:           strconv.Itoa(int(AuthID)),
			updateJSON:   `{"nickname":"Frank", "email": "rsteven@jap.com", "password": "password"}`,
			statusCode:   500,
			tokenGiven:   tokenString,
			errorMessage: "Email Already Taken",
		},
		{
			...
			id:           strconv.Itoa(int(AuthID)),
			updateJSON:   `{"nickname":"Ronald Steven", "email": "grsteven@jap.com", "password": "password"}`,
			statusCode:   500,
			tokenGiven:   tokenString,
			errorMessage: "Nickname Already Taken",
		},
